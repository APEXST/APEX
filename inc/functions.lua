...
download_file('https://raw.githubusercontent.com/APEXST/APEX/master/inc/Run.lua','./inc/Run.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'40% - |█████       |') end
download_file('https://raw.githubusercontent.com/APEXST/APEX/master/inc/locks.lua','./inc/locks.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'60% - |███████     |') end
download_file('https://raw.githubusercontent.com/APEXST/APEX/master/inc/Script.lua','./inc/Script.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'80% - |█████████   |') end
download_file('https://raw.githubusercontent.com/APEXST/APEX/master/inc/functions.lua','./inc/functions.lua')
if edit then EditMsg(msg.chat_id_,msg.id_,'100% - |█████████████|\n\n🔝*¦* تم تحديث السورس الى اصدار *v'..redis:get(APEX..":VERSION")..'*\n📟*¦* تم اعاده تشغيل السورس بنجاح') end
if edit then dofile("./inc/Run.lua") end
print("Update Source And Reload ~ ./inc/Run.lua")

end
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(APEX..":NameBot:") or "الزعيم"
...
usernn = data.username_ or "APEXST"
else 
Name = data.title_
usernn = data.type_.user_.username_ or "APEXST"
end
Name = Name:gsub('[[][]]','')
...
var = 'ادمن في البوت 👨🏼‍🎓' 
elseif redis:sismember(APEX..'whitelist:'..ChatID,UserID) then
var = 'عضو مميز ⭐️' 
else
var = 'فقط عضو 🙍🏼‍♂️' 
end
...
function convert_Klmat(msg,data,Replay,MD)
  ...
  Replay = Replay:gsub("{البوت}",redis:get(APEX..':NameBot:'))
...
if msg.edited then
redis:incr(APEX..':edited:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.text and not msg.forward_info_ then
redis:incr(APEX..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_)
elseif msg.content_.ID == "MessageChatAddMembers" then 
redis:incr(APEX..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessagePhoto" then
redis:incr(APEX..':photo:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageSticker" then
redis:incr(APEX..':sticker:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageVoice" then
redis:incr(APEX..':voice:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageAudio" then
redis:incr(APEX..':audio:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageVideo" then
redis:incr(APEX..':video:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.content_.ID == "MessageAnimation" then
redis:incr(APEX..':animation:'..msg.chat_id_..':'..msg.sender_user_id_)
end
end
...
function TagAll(msg)
  ...
  local monshaas = redis:smembers(APEX..':MONSHA_Group:'..msg.chat_id_)
  local monsha = redis:smembers(APEX..':MONSHA_BOT:'..msg.chat_id_)
  local Owners = redis:smembers(APEX..'owners:'..msg.chat_id_)
  local Admins = redis:smembers(APEX..'admins:'..msg.chat_id_)
  local mmez = redis:smembers(APEX..'whitelist:'..msg.chat_id_)
...
for k,v in pairs(mmez) do
  ...
  local info  = redis:hgetall(APEX..'username:'..v)
  if info and info.username and info.username:match("@[%a%d_]+") then
  message = message ..i.."-l ["..info.username..'] \n'
  else
  message = message ..i.. '-l ['..info.username..'](t.me/APEXST) \n'
  end
...
local list = redis:smembers(APEX..':SUDO_BOT:')
message = '👨🏽‍💻*¦* قائمه الـمـطـوريـن : \n\n`★`*_* ['..SUDO_USER..'] ➣ (' ..SUDO_ID.. '){'..redis:scard(APEX..'mtwr_count'..SUDO_ID)..'}\n*----------------------------------*\n'
if #list==0 then  message = message.."* لا يوجد مطورين حاليا \n📛 *"
else
for k,v in pairs(list) do
local info  = redis:hgetall(APEX..'username:'..v)
local count = redis:scard(APEX..'mtwr_count'..v)
if info and info.username and info.username:match("@[%a%d_]+") then
message = message ..k.."-l ["..info.username..'] » (`' ..v.. '`){'..count..'} \n'
else
message = message ..k.. '-l ['..info.username..'](t.me/APEXST) l » (`' ..v.. '`){'..count..'} \n'
end
end 
end
...
function conslist(msg)
    ...
    local monsha = redis:smembers(APEX..':MONSHA_Group:'..msg.chat_id_)
    if #monsha == 0 then 
    ...
    message = message ..k.."-l ["..info.username..'] » (`' ..v.. '`) \n'
      ...
      message = message ..k.. '-l ['..info.username..'](t.me/APEXST) l » (`' ..v.. '`) \n'
    end
    end
    end
...
local list = redis:smembers(APEX..'owners:'..msg.chat_id_)
if #list == 0 then  
...
local list = redis:smembers(APEX..'admins:'..msg.chat_id_)
...
local list = redis:smembers(APEX..'whitelist:'..msg.chat_id_)
...
if redis:sismember(APEX..'is_silent_users:'..Group,User) then 
return true 
else
return false
end 
...
local list = redis:smembers(APEX..'is_silent_users:'..msg.chat_id_)
...
if redis:sismember(APEX..'banned:'..Group,User) then 
return true 
else
return false
end 
...
if redis:sismember(APEX..'gban_users',User) then 
return true 
else
return false
end 
...
local list = redis:smembers(APEX..'gban_users')
...
if not msg.Admin and not msg.Special then -- للاعضاء فقط  
  local list = redis:smembers(APEX..':Filter_Word:'..msg.chat_id_)
if #list ~=0 then
...
if redis:sismember(APEX..':Filter_Word:'..msg.chat_id_,word) then 
return  "📝*¦* الكلمه *{"..word.."}* هي بالتأكيد من قائمه المنع✓️" 
else
redis:sadd(APEX..':Filter_Word:'..msg.chat_id_,word) 
...
if redis:sismember(APEX..':Filter_Word:'..msg.chat_id_,word) then 
redis:srem(APEX..':Filter_Word:'..msg.chat_id_,word) 
...
local NumberFlood = tonumber(redis:get(APEX..':Flood_Spam:'..User..':'..ChatID..':msgs') or 0)
if NumberFlood >= 5 then 
result = false
else
redis:setex(APEX..':Flood_Spam:'..User..':'..ChatID..':msgs',2,NumberFlood+1)
result = true
end
return result
end
...
json_data = '{"BotID": '..APEX..',"UserBot": "'..Bot_User..'","Groups" : {'
local All_Groups_ID = redis:smembers(APEX..'group:ids')
for key,GroupS in pairs(All_Groups_ID) do
local NameGroup = (redis:get(APEX..'group:name'..GroupS) or '')
...
local admins = redis:smembers(APEX..'admins:'..GroupS)
if #admins ~= 0 then
json_data =  json_data..',"Admins" : {'
for key,value in pairs(admins) do
local info = redis:hgetall(APEX..'username:'..value)
if info then 
...
local creator = redis:smembers(APEX..':MONSHA_BOT:'..GroupS)
if #creator ~= 0 then
...
local owner = redis:smembers(APEX..'owners:'..GroupS)
if #owner ~= 0 then
...
local list = redis:smembers(APEX..'group:ids')
message = '📋*¦* قائمه المجموعات :\n\n'
for k,v in pairs(list) do 
local info = redis:get(APEX..'group:name'..v)
if info then 
...
redis:del(
APEX..'group:add'..id_group,
APEX..'lock_link'..id_group, 
APEX..'lock_id'..id_group,
APEX..'lock_spam'..id_group, 
APEX..'lock_webpage'..id_group,
APEX..'lock_markdown'..id_group,
APEX..'lock_flood'..id_group,
APEX..'lock_bots'..id_group,
APEX..'mute_forward'..id_group,
APEX..'mute_contact'..id_group,
APEX..'mute_location'..id_group,
APEX..'mute_document'..id_group,
APEX..'mute_keyboard'..id_group,
APEX..'mute_game'..id_group,
APEX..'mute_inline'..id_group,
APEX..'lock_username'..id_group,
APEX..'num_msg_max'..id_group,
APEX..'mute_text'..id_group,
APEX..'admins:'..id_group,
APEX..':Filter_Word:'..id_group,
APEX..'banned:'..id_group,
APEX..'is_silent_users:'..id_group,
APEX..'whitelist:'..id_group,
APEX..':MONSHA_BOT:'..id_group,
APEX..'owners:'..id_group,
APEX..'replay'..id_group,
APEX..':MONSHA_Group:'..id_group
)
redis:srem(APEX..'group:ids',id_group)
end
...
NumAdmin = NumAdmin + 1
if not redis:sismember(APEX..'admins:'..arg.chat_id_,v.user_id_) then
GetUserID(v.user_id_,function(arg,data)
redis:hset(APEX..'username:'..data.id_,'username',ResolveUserName(data))
redis:sadd(APEX..'admins:'..arg.chat_id_,data.id_)
...
if redis:get(APEX..'group:add'..msg.chat_id_) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
local GroupUsers = tonumber(redis:get(APEX..':addnumberusers') or 0)
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(APEX..'username:'..data.id_,'username', ResolveUserName(data))
redis:sadd(APEX..':MONSHA_Group:'..arg.chat_id_,data.id_)
...
redis:mset(
APEX..'group:add'..arg.chat_id_,true,
APEX..'lock_link'..arg.chat_id_,true, 
APEX..'lock_id'..arg.chat_id_,true,
APEX..'lock_spam'..arg.chat_id_,true,
APEX..'lock_webpage'..arg.chat_id_,true,
APEX..'lock_markdown'..arg.chat_id_,true,
APEX..'lock_flood'..arg.chat_id_,true,
APEX..'lock_bots'..arg.chat_id_,true,
APEX..'mute_forward'..arg.chat_id_,true,
APEX..'mute_contact'..arg.chat_id_,true,
APEX..'mute_location'..arg.chat_id_,true,
APEX..'mute_document'..arg.chat_id_,true,
APEX..'mute_keyboard'..arg.chat_id_,true,
APEX..'mute_game'..arg.chat_id_,true,
APEX..'mute_inline'..arg.chat_id_,true,
APEX..'lock_username'..arg.chat_id_,true,
APEX..'num_msg_max'..arg.chat_id_,5, 
APEX..'lock_edit'..arg.chat_id_,true,
APEX..'replay'..arg.chat_id_,true,
APEX.."lock_KickBan"..msg.chat_id_,true,
APEX.."lock_mmno3"..msg.chat_id_,true,
APEX.."lock_KickBan"..msg.chat_id_,true,
APEX.."lock_linkk"..msg.chat_id_,true
)
redis:sadd(APEX..'group:ids',arg.chat_id_) 
redis:sadd(APEX..'mtwr_count'..arg.sender_user_id_,arg.chat_id_)

local NameGroup = data.title_
redis:set(APEX..'group:name'..arg.chat_id_,NameGroup)
...
redis:set(APEX..'linkGroup'..arg.chat_id_,Gp_Link)
...
if cmd == "Upmonsh" then
if redis:sismember(APEX..':MONSHA_Group:'..ChatID,UserID) then 
...
redis:sadd(APEX..':MONSHA_Group:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📮 » تم رفع ❲ "..NameUser.." ❳ منشئ اساسي في المجموعة \n✓️") 
end

if cmd == "Dwmonsh" then
if not redis:sismember(APEX..':MONSHA_Group:'..ChatID,UserID) then 
sendMsg(ChatID,MsgID,"📮 » تم بالتأكيد تنزيل ❲ "..NameUser.." ❳ منشئ اساسي في المجموعة \n✓️") 
end
redis:srem(APEX..':MONSHA_Group:'..ChatID,UserID) 
...
redis:hset(APEX..'username:'..UserID, 'username', Resolv)
redis:sadd(APEX..'banned:'..ChatID,UserID)
...
redis:hset(APEX..'username:'..UserID, 'username', Resolv)
redis:srem(APEX..'banned:'..ChatID,UserID)
StatusLeft(ChatID,UserID)
return false
end
...
redis:hset(APEX..'username:'..UserID, 'username', Resolv)
redis:sadd(APEX..'is_silent_users:'..ChatID,UserID)
...
redis:hset(APEX..'username:'..UserID, 'username', Resolv)
redis:srem(APEX..'is_silent_users:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,"📌 » العضو ❲ "..NameUser.." ❳  تم الغاء كتمه من المجموعة\n✓") 
end

if cmd == "upMshrf" then
  redis:hset(APEX..'username:'..UserID,'username',Resolv)
  redis:setex(APEX..":uploadingsomeon:"..ChatID..msg.sender_user_id_,500,NameUser)
  redis:setex(APEX..":uploadingsomeon2:"..ChatID..msg.sender_user_id_,500,UserID)
...
if cmd == "tfa3l" then  
  local maseegs = redis:get(APEX..'msgs:'..UserID..':'..ChatID) or 1
  local edited = redis:get(APEX..':edited:'..ChatID..':'..UserID) or 0
  local content = redis:get(APEX..':adduser:'..ChatID..':'..UserID) or 0
  if data.username_ then UserNameID = "@"..data.username_ else UserNameID = "لا يوجد" end  
...
.."\n📝¦ التعديل » "..(redis:get(APEX..'lock_edit'..msg.chat_id_) or 'false')
.."\n🔗¦ الروابط » "..(redis:get(APEX..'lock_link'..msg.chat_id_) or 'false')
.."\n#️⃣¦ التاك » "..(redis:get(APEX..'lock_tag'..msg.chat_id_) or 'false')
.."\n©¦ المعرفات » "..(redis:get(APEX..'lock_username'..msg.chat_id_) or 'false')
.."\n\n💱¦ التكرار » "..(redis:get(APEX..'lock_flood'..msg.chat_id_) or 'false')
.."\n📑¦ الكلايش » "..(redis:get(APEX..'lock_spam'..msg.chat_id_) or 'false')
.."\n🌐¦ الويب » "..(redis:get(APEX..'lock_webpage'..msg.chat_id_) or 'false')
.."\n⚜️¦ الماركدوان » "..(redis:get(APEX..'lock_markdown'..msg.chat_id_) or 'false')
.."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(APEX..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n🤖¦ البوتات » "..(redis:get(APEX..'lock_bots'..msg.chat_id_) or 'false')
.."\n➕¦ عدد التكرار » "..(redis:get(APEX..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n🎬¦` اعدادات الوسائط :`\n"
.."\n🤹*¦* المتحركه » "..(redis:get(APEX..'mute_gif'..msg.chat_id_) or 'false')
.."\n💭¦ الدردشه » "..(redis:get(APEX..'mute_text'..msg.chat_id_) or 'false')
.."\n◽️¦ الانلاين » "..(redis:get(APEX..'mute_inline'..msg.chat_id_) or 'false')
.."\n🎮¦ الالعاب » "..(redis:get(APEX..'mute_game'..msg.chat_id_) or 'false')
.."\n🏞¦ الصور » "..(redis:get(APEX..'mute_photo'..msg.chat_id_) or 'false')
.."\n🎥¦ الفيديو » "..(redis:get(APEX..'mute_video'..msg.chat_id_) or 'false')
.."\n🎙¦ الصوت » "..(redis:get(APEX..'mute_audio'..msg.chat_id_) or 'false')
.."\n\n🔉¦ البصمات » "..(redis:get(APEX..'mute_voice'..msg.chat_id_) or 'false')
.."\n🎎¦ الملصقات » "..(redis:get(APEX..'mute_sticker'..msg.chat_id_) or 'false')
.."\n📞¦ الجهات » "..(redis:get(APEX..'mute_contact'..msg.chat_id_) or 'false')
.."\n💱¦ التوجيه » "..(redis:get(APEX..'mute_forward'..msg.chat_id_) or 'false')
.."\n🌐¦ الموقع » "..(redis:get(APEX..'mute_location'..msg.chat_id_) or 'false')
.."\n🗂¦ الملفات » "..(redis:get(APEX..'mute_document'..msg.chat_id_) or 'false')
.."\n🔅¦ الاشعارات » "..(redis:get(APEX..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n🔒¦ الفشار » "..(redis:get(APEX..'lock_mmno3'..msg.chat_id_) or 'false')
.."\n🔒¦ الفارسيه » "..(redis:get(APEX..'lock_pharsi'..msg.chat_id_) or 'false')
.."\n🔒¦ الانكليزيه » "..(redis:get(APEX..'lock_lang'..msg.chat_id_) or 'false')
.."\n🔒¦ الاضافه » "..(redis:get(APEX..'lock_Add'..msg.chat_id_) or 'false')

local eueuf = "\n\n*⚒¦*` اعدادات اخرى : `"
.."\n*🙋🏼‍♂️¦* الترحيب » "..(redis:get(APEX..'welcome:get'..msg.chat_id_) or 'false')
.."\n*💬¦*  الردود » "..(redis:get(APEX..'replay'..msg.chat_id_) or 'false')
.."\n*🚸¦*  التحذير » "..(redis:get(APEX..'lock_woring'..msg.chat_id_) or 'false')
.."\n*🎫¦* الايدي » "..(redis:get(APEX..'lock_id'..msg.chat_id_) or 'false')
.."\n*🎫¦* الرابط » "..(redis:get(APEX..'lock_linkk'..msg.chat_id_) or 'false')
.."\n*🎫¦* المغادره » "..(redis:get(APEX..'lock_leftgroup'..msg.chat_id_) or 'false')
.."\n*🎫¦* الحظر » "..(redis:get(APEX..'lock_KickBan'..msg.chat_id_) or 'false')
.."\n*🎫¦* الحمايه » "..(redis:get(APEX..'antiedit'..msg.chat_id_) or 'false')
.."\n*🎫¦* التاك للكل » "..(redis:get(APEX..'lock_takkl'..msg.chat_id_) or 'false')
.."\n*🎫¦* الايدي بالصوره » "..(redis:get(APEX..'idphoto'..msg.chat_id_) or 'false')
.."\n*🎫¦* التحقق » "..(redis:get(APEX.."lock_check"..msg.chat_id_) or 'false')
...
.."\n*➕¦* عدد التكرار » "..(redis:get(APEX..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n*💱¦*` اعدادات التقـييد :`\n"
.."\n*💢¦* التقييد بالتوجيه » "..(redis:get(APEX..':tqeed_fwd:'..msg.chat_id_) or 'false')
.."\n*📸¦* التقييد بالصور » "..(redis:get(APEX..':tqeed_photo:'..msg.chat_id_) or 'false')
.."\n*🔗¦* التقييد بالروابط » "..(redis:get(APEX..':tqeed_link:'..msg.chat_id_) or 'false')
.."\n*🎉¦* التقييد بالمتحركه » "..(redis:get(APEX..':tqeed_gif:'..msg.chat_id_) or 'false')
.."\n*🎥¦* التقييد الفيديو » "..(redis:get(APEX..':tqeed_video:'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
...
.."\n*🗂¦* الملفات » "..(redis:get(APEX..'mute_document'..msg.chat_id_) or 'false')
.."\n*🔅¦* الاشعارات » "..(redis:get(APEX..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n*🔒¦* الكيبورد » "..(redis:get(APEX..'mute_keyboard'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
return sendMsg(msg.chat_id_,msg.id_,'➖\n'..list_settings..'\n')
end
